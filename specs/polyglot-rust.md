...
- [ ] `okay-rust/kernels/argon2` is a Cargo crate, `cdylib` and
      ...
      and works offline.
- [ ] `okay.rust.NativeLib.load(path)` binds a library through FFM. A
      symbol it does not export is refused by name, not a crash.
- [ ] `enum Kdf[+A] derives okay.Effect` has one operation,
      ...
      - `Kdf.using(f)` is the handler over any function (a test's, or a
        JVM implementation).
- [ ] THE LAW (Live: needs cargo): for the same inputs, the Rust kernel's
...
- [ ] A program written against `Kdf` runs under either handler
      unchanged.
