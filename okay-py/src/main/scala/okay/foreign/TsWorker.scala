...
object TsWorker:
  ...
  private def resource(name: String): String =
    ...
    if res == null then throw IllegalStateException(s"okay.foreign: /okay/ts/$name is missing from the jar")
  ...
  def command(dir: Path, modules: Seq[String], node: String = "node",
    ...
    modules.foreach { m =>
      if !Files.exists(dir.resolve(s"$m.ts")) then
        throw IllegalArgumentException(s"okay.foreign: no module $m.ts in $dir")
    }
