...
object RustWorker:
  ...
  private def resource(path: String): String =
    ...
    if res == null then throw IllegalStateException(s"okay.foreign: /okay/rust-crate/$path is missing from the jar")
  ...
  def build(dir: Path, cargo: String = "cargo", features: Seq[String] = Nil): Path =
    ...
    val p =
      try pb.start()
        ...
        throw IllegalStateException(s"okay.foreign: '$cargo' did not start (${e.getMessage}) — is Rust installed?")
    val log = String(p.getInputStream.readAllBytes(), java.nio.charset.StandardCharsets.UTF_8)
    if p.waitFor() != 0 then
      throw IllegalStateException(s"okay.foreign: the Rust worker did not compile:\n${log.linesIterator.toVector.takeRight(25).mkString("\n")}")
  ...
  def buildLibrary(dir: Path, target: Option[String] = None, cargo: String = "cargo", features: Seq[String] = Nil): Path =
    ...
    val log = String(p.getInputStream.readAllBytes(), java.nio.charset.StandardCharsets.UTF_8)
    if p.waitFor() != 0 then
      throw IllegalStateException(s"okay.foreign: the Rust library did not compile:\n${log.linesIterator.toVector.takeRight(25).mkString("\n")}")
    target match
...
object Rs:
  ...
  def ops[F[+_]](cbs: Foreign.Callbacks[F]): String =
    ...
    s"""// Code generated by okay.foreign.Rs from the Scala callbacks. DO NOT EDIT.
       |#![allow(dead_code)]
       |
