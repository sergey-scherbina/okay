...
- [ ] growing-order-drain-guarantee — WHAT THE PROBE LEFT. `Growing`
      promises per-producer FIFO and cannot keep it when a producer's
      elements straddle the swap: part 0 is drained first per SCAN,
      not globally, so an element already in part 0 can be overtaken
      by later ones in part 1. Two candidate fixes, both HOT PATH and
      therefore both needing a number before they land: drain part 0
      to empty before adopting it, or hold a producer in part 0 until
      the consumer has passed it. Reproducer and evidence:
      `ProbeGrowingOrder`, BUGS.md.
