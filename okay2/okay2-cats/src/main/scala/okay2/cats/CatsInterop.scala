...
object CatsInterop extends CatsInteropLow {
  ...
  trait Into[R <: Row, M[_]] { def apply[X](e: R#Op[X]): M[X] }

  object Into {
    /** a union is interpreted by its parts: split by the F test,
     * exactly as `Handler.union` */
    implicit def union[F <: Row, G <: Row, M[_]](implicit T: TypeableK[F], f: Into[F, M], g: Into[G, M]): Into[F + G, M] =
      new Into[F + G, M] {
        def apply[X](e: (F + G)#Op[X]): M[X] = Split.split[F, G, X, M[X]](e)(f(_))(g(_))
      }

    /** Pure has no operations: never applied */
    implicit def pure[M[_]]: Into[okay2.Pure, M] = new Into[okay2.Pure, M] {
      def apply[X](e: Nothing): M[X] = e
    }
  }
   ...
   */
  def foldTo[M[_], A, R <: Row](p: A ! R)(h: Into[R, M])(implicit M: Monad[M]): M[A] =
    M.tailRecM[A ! R, A](p) { x =>
      Free.resume(x) match {
        case Return(a) => M.pure(Right(a))
        case Inject(e) => M.map(h(e))(a => Right(a))
        case Bind(Inject(e), k) => M.map(h(e))(v => Left(k(v)))
        case other => throw new IllegalStateException("resume left a non-head form: " + other)
      }
    }

  /** the same, into IO, for the row whose parts all have an `Into[_, IO]` */
  def toIO[A, R <: Row](p: A ! R)(implicit h: Into[R, IO]): IO[A] = foldTo[IO, A, R](p)(h)

  /** the tree over cats' own `Free`: the same nodes, cats' `Suspend`
   * and `FlatMapped` for our `Inject` and `Bind` */
  def toCats[R <: Row, A](p: A ! R): CFree[R#Op, A] = Free.resume(p) match {
    case Return(a) => CFree.pure(a)
    case Inject(e) => CFree.liftF[R#Op, A](e)
    case Bind(Inject(e), k) => CFree.liftF[R#Op, Any](e).flatMap(x => toCats[R, A](k(x)))
    case other => throw new IllegalStateException("resume left a non-head form: " + other)
  }
...
object instances extends CatsInteropLow {
  ...
  implicit def programMonadError[E, F <: Row]: MonadError[({ type L[A] = A ! (Throws[E] + F) })#L, E] =
    new MonadError[({ type L[A] = A ! (Throws[E] + F) })#L, E] with StackSafeMonad[({ type L[A] = A ! (Throws[E] + F) })#L] {
      ...
      def flatMap[A, B](fa: A ! (Throws[E] + F))(f: A => B ! (Throws[E] + F)): B ! (Throws[E] + F) = fa.flatMap(f)
      def raiseError[A](e: E): A ! (Throws[E] + F) = Throws.raise[E, A](e).at[Throws[E] + F]
      def handleErrorWith[A](fa: A ! (Throws[E] + F))(f: E => A ! (Throws[E] + F)): A ! (Throws[E] + F) = fa.recover(f)
    }
}
...
trait CatsInteropLow {
  ...
  implicit def programMonad[R <: Row]: StackSafeMonad[({ type L[A] = A ! R })#L] =
    new StackSafeMonad[({ type L[A] = A ! R })#L] {
      ...
      def flatMap[A, B](fa: A ! R)(f: A => B ! R): B ! R = fa.flatMap(f)
    }
}
...
object Io {
  ...
  implicit val into: Into[Io, IO] = new Into[Io, IO] {
  ...
  def run[A, R <: Row](p: A ! R)(implicit h: Into[R, IO]): IO[A] = foldTo[IO, A, R](p)(h)
}
