 ...
 * `SparkSchema.MaxNesting` (64, Arrow's own limit, the one okay-arrow
 * keeps) before any walk starts, so the walks recurse at most that deep.
 */
class TestSparkDepth extends munit.FunSuite:
  ...
  test("a type 64 levels deep converts, and so does a value of it") {
    val t = arrays(SparkSchema.MaxNesting)
    var d: DataType = SparkSchema.dataType(t)
    var levels = 1
    var more = true
    while more do d match
      case ArrayType(e, _) => d = e; levels += 1
      case StructType(Array(f)) => d = f.dataType; levels += 1
      case _ => more = false
    assertEquals(levels, SparkSchema.MaxNesting)
    assertEquals(d, LongType)
    assertEquals(SparkSchema.value(ColType.Arr(ColType.Int64, true), Vector(1L, 2L)), Vector(1L, 2L))
  }

  test("a type past 64 levels is refused by name, however deep") {
    for depth <- Vector(SparkSchema.MaxNesting + 1, 200000) do
      val t = arrays(depth)
