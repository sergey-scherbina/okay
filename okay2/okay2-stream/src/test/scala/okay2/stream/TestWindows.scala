...
/**
 * Event-time windows, against a brute force that windows the same
 * events by recomputing every pane from scratch: the operator is a
 * fold with a watermark and an eviction sweep; the reference is a
 * groupBy, which is slow, obviously right, and the only kind of
 * oracle worth having here.
 */
/** an event: at the top level, where a case class's outer reference needs no run-time check */
...
class TestWindows extends munit.FunSuite {
  ...
  /** splitmix64: the same stream everywhere, no Random */
  def mix(x: Long): Long = {
  ...
  test("tumbling windows equal a recompute of every pane") {
    ...
    for (p <- panes) assertEquals(p.end - p.start, 100L)
  }
  ...
  test("an element later than every window it belongs to is dropped and counted") {
    ...
    w.add(Ev(100, "a", 7))(p => { out += p; () })
    assertEquals(out.result(), Vector(Pane(0L, 10L, "a", 1L)))
    w.add(Ev(5, "a", 999))(p => { out += p; () })
  ...
  test("an element inside the lateness bound is not dropped, however out of order") {
    ...
    w.add(Ev(60, "a", 2))(p => { out += p; () })
    assertEquals(w.dropped, 0L)
    w.close()(p => { out += p; () })
    assertEquals(out.result().sortBy(_.start), Vector(Pane(60L, 70L, "a", 2L), Pane(100L, 110L, "a", 1L)))
  }
  ...
  test("one BUILT program, run twice: the same panes — `into` starts the stage when the program RUNS") {
  ...
  test("a window is a stage like any other: it composes under `through`") {
    ...
    assertEquals(told.sorted, panes.map(_.value).sorted)
    assertEquals(total, panes.map(_.value).sum)
  }
}
