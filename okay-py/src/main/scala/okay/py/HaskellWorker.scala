  ...
  def build(dir: Path, main: String = "Main.hs", ghc: String = "ghc"): Path =
    ...
    val p =
      try pb.start()
        ...
        throw IllegalStateException(s"okay.py: '$ghc' did not start (${e.getMessage}) — is GHC installed?")
    val log = String(p.getInputStream.readAllBytes(), java.nio.charset.StandardCharsets.UTF_8)
    if p.waitFor() != 0 then
      throw IllegalStateException(s"okay.py: the Haskell worker did not compile:\n${log.linesIterator.toVector.takeRight(20).mkString("\n")}")
    bin
