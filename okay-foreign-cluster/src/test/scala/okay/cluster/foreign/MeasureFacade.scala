...
class MeasureFacade extends munit.FunSuite:
  ...
  test("tier 1, 2 and 3 through the facade, python3 and the JVM, beside their own roads") {
    ...
    val frameOwn = median(5)(PyPool.frame(pool, python, "facademeasure:fecho", ownFrame, Vector.empty))
    ...
    println(f"  python tier 2 ($n rows, one frame): facade $frameFacade%8.1f ms   own road $frameOwn%8.1f ms (facade adds Rows.table + Rows.rows)")
