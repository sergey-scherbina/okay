...
| `limiter.pace(key)(sleep)` | a BLOCKING caller's pace: the token taken now, the thread sleeps for its turn, never refused; with `burst = 1` a fixed interval (okay-watch's request pace) | — |
