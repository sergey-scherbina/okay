...
import okay2.codec.{JsonSchema, Schema}
import okay2.http.syntax._
import Drive.run

/** a route declares what it requires, and the table enforces it
 * (okay-http's TestRouteSecured): `enforcing` refuses exactly the
 * entries whose `security` is non-empty, which is exactly the set a
 * renderer calls protected — one value, two interpreters */
class TestRouteSecured extends munit.FunSuite {

  private def ok(s: String): Response ! Async = pure[Async, Response](Response(200, Nil, Http.one(s.getBytes("UTF-8"))))
  ...
  private val verify: Router.Verify = t =>
    if (t.startsWith("bad")) Left("invalid")
  ...
  private def status(r: PartialFunction[Request, Response ! Async], req: Request): Int = run(r(req)).status

  private def header(r: PartialFunction[Request, Response ! Async], req: Request, n: String) =
    run(r(req)).headers.collectFirst { case (k, v) if k.equalsIgnoreCase(n) => v }

  private def post(token: Option[String]) =
    Request.post("/admin/replay", Body.Empty, token.map(t => "authorization" -> s"Bearer $t").toSeq)
  ...
  test("no credential is 401, and the route still MATCHES, which is the point") {
    ...
    assertEquals(header(r, post(None), "www-authenticate"), Some("""Bearer realm="okay", error="no token""""))
  }

  test("a credential that does not verify is 401, and says no more than that") {
    val r = table.enforcing(verify).routes
    assertEquals(status(r, post(Some("bad:admin"))), 401)
    assertEquals(header(r, post(Some("bad:admin")), "www-authenticate"), Some("""Bearer realm="okay", error="invalid_token""""))
  }

  test("verified and not permitted is 403, a different fact from 401") {
    assertEquals(status(table.enforcing(verify).routes, post(Some("t:reader"))), 403)
  }
  ...
  test("a REFUSED request never runs the handler") {
    val r = table.enforcing(verify).routes
    ran = 0
    status(r, post(None)): Unit
    status(r, post(Some("bad:admin"))): Unit
    status(r, post(Some("t:reader"))): Unit
  ...
  test("FAIL CLOSED: a secured entry with no verifier does not serve") {
    ...
    assertEquals(header(r, post(Some("t:admin")), "www-authenticate"), Some("""Bearer realm="okay", error="no_verifier""""))
  }

  test("an UNSECURED route in the same table is untouched, enforced or not") {
    for (r <- Seq(table.routes, table.enforcing(verify).routes)) assertEquals(status(r, Request.get("/healthz")), 200)
  }
  ...
  test("THE LAW: what the table refuses is what the description calls protected") {
    ...
    val refused = t.entries.map { e =>
      ...
      (e.method, e.path, t.routes.lift(req).map(p => run(p).status))
  ...
  test("the challenge header is DECLARED, and it is the one the wire carries") {
    ...
    assert(header(table.enforcing(verify).routes, post(None), "www-authenticate").isDefined, "declared and not sent")
  }

  test("an author's declared response header is DESCRIPTION, and attaches to the status") {
    val r = Router.on(Method.Get, Route / "t")(_ => ok("x")).answering(200, "etag".as[String])
  ...
  test("answering an empty table throws, here, where the mistake is") {
    intercept[IllegalStateException](Router.empty.answering(200, "etag".as[String])): Unit
  }

  test("out: a secured route declares its VALUE beside its refusals") {
    val r = Router.out(Method.Get, (Route / "t" / "id".as[Int]).secured("admin"), 200, "the task")((id: Int, _: Unit) => pure[Async, Task](Task(id, "x")))
    val e = r.entries.head
    assertEquals(e.answers.map(_.status).sorted, Vector(200, 401, 403))
    assertEquals(e.answers.find(_.status == 200).flatMap(_.schema), Some(JsonSchema.of(implicitly[Schema[Task]])))
  ...
  test("jsonOut: a body in, a value out, and the refusals, all declared") {
    val r = Router.jsonOut[Unit, Unit, NewTask, Task](Method.Post, (Route / "t").secured("admin"), 201, "the task created")(
      (_, _, n) => pure[Async, Task](Task(1, n.title)))
    val e = r.entries.head
  ...
  test("events: the shape `last-event-id` was declared for") {
    ...
    var seen: Any = null
    val r = Router.events(Method.Get, resume, 200, "the stream")((_, from) => { seen = from; pure[Async, okay2.stream.Source[okay2.stream.Chunk[Byte]]](Http.one(Array.empty[Byte])) })
  ...
  test("bytes and htmlAt reach a declaring route too") {
    val b = Router.bytes(Method.Get, (Route / "b").secured("admin"), "application/pdf", 200, "a pdf")((_, _) => pure[Async, Array[Byte]](Array.empty[Byte]))
    assertEquals(b.entries.head.answers.find(_.status == 200).map(_.media), Some("application/pdf"))
    val h = Router.htmlAt(Method.Get, (Route / "h").secured("admin"), 200, "a page")((_, _, _) => pure[Async, String]("<p>x</p>"))
