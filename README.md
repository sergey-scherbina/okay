...
| [Foreign languages](docs/foreign-languages.md) | the one model under every language okay runs — Python, R, TypeScript, Go, Rust, Haskell in their own process, Clojure, Frege and Java in the JVM: what crosses (a value, a table, an object, a stream, a program), one program in every language over the same Scala, who carries what, the wire's five operations and its transcript, adding a language; the pages below are its chapters |
...
| [One facade over every foreign language](docs/foreign-facade.md) | a job names a module and a function, and a typeclass by the module's type says who runs it — Python, R, the JVM's own — and how the data crosses: a value as a call, rows as one frame (Arrow where spoken, by reference on the JVM), a Flow as frames; what a language cannot do does not compile; programs as data and held objects through the same shape; every tier measured beside the language's own road |
