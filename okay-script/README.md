...
Markdown files as Scala source (specs/okay-script.md; the road from an empty directory is [the guide](../okay-script-guide.md)). A `.md` file with fenced ` ```scala ` blocks is a literate program: the blocks are extracted, concatenated in document order, and compiled by the REAL Scala 3 compiler in-process (`dotty.tools.dotc`), then run. No new language and no interpreter — markup metadata extraction, minimal preprocessing, meta-compilation.
