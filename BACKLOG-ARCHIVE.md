...
Closed entries moved out of `BACKLOG.md` on 2026-09-10 (backlog-cleanup),
verbatim and in their original order, sections and all. Landed work is also
in `CHANGELOG.md`; what is ONLY here is the refuted, the declined and the
measured-and-abandoned — which is why this file exists instead of a delete.

Open items were moved to `BACKLOG.md` and grouped by module; a section that
held one says so there.
...
## spark-structured-streaming-lane — Spark's own event-time engine, unmeasured
...
- [x] optics-outside-topology — ANSWERED (moved 2026-09-19). A
      dataflow is an arrow: the ONLY one of the six that meets the
      staticness condition the spec sets for reaching for `Arrow` at
      all — the graph must exist as a value before it runs, to be
      drawn, fused, or shipped to a cluster. Wants `ArrowChoice` or
...
- [x] effect-instances-tunnelling — PRIORITY: LOW (design; trigger). a known limit, now named after its
      ...
      CLOSED 2026-09-25 by lexical-instances (stages 0-3): `okay.Lexical`
      has effect instances as prompts (POPL 2020), with no accidental
      handling. Every strategy is named (tail, deep, shallow, stacked deep
      and tail), and a default picks by clause kind. The price on the hot
...
- [x] lexical-tagged-walk — PRIORITY: LOW (design + measurement; trigger). The +72 B per
      ...
      first. (2026-09-25)
      DONE 2026-09-25 as the OPTIONAL `Lexical.walk` (operator: "optionally,
      not the default"): 1.29x the row's bytes against tail's 1.65x. The
      caveat above was REFUTED by its tests. Walk never answers silently
      differently: across the installation it gives deep's answer, and
