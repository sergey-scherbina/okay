...
trait PasswordHashPlatform:
    ...
    PasswordHash.using(op => lib.withBuffers { b =>
      ...
      lib.call("okay_argon2id",
        ...
        .flatMap { code =>
          ...
          else Left(s"okay_argon2id answered ${code.toInt}: ${PasswordHash.meaning(code.toInt)}")
        }
    })
  ...
  private def call(mh: MethodHandle, op: PasswordHash.Argon2id): Either[String, Array[Byte]] =
    ...
    try
      ...
      answer match
        ...
        case code: java.lang.Integer => Left(s"okay_argon2id answered ${code.intValue}: ${PasswordHash.meaning(code.intValue)}")
