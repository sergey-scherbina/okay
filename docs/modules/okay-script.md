...
Markdown files as Scala source (specs/okay-script.md; the road from
an empty directory is [the guide](../okay-script-guide.md)). A `.md` file
...
| HTTPS (ACME) | `OKAY_ACME=<email>` + `OKAY_ACME_DOMAINS=a,b` asks a CA for the certificate itself (okay-acme, HTTP-01, staging by default) |
| HTTPS | `OKAY_TLS=self` (a self-signed certificate generated once), or `OKAY_TLS_CERT`+`OKAY_TLS_KEY`; behind a proxy `OKAY_FORWARDED=1` plus `OKAY_HSTS` and `OKAY_HTTPS_ONLY`; `OKAY_HTTP_PORT` is a plaintext port that only redirects |
...
| `Site.warm` / `Site.stats` / `opsRouter` / `opsRoutes` / `Site.Ops.paths` | compile the whole directory at boot (a broken page is named then, not by the first visitor), counters and gauges, and opt-in `/healthz` `/stats` `/metrics` in JSON and Prometheus text — declared as values in `Site.Ops`, which is where `ScriptDeploy` reads its probe paths from instead of writing literals (specs/optics-outside.md, stage 4) |
...
| `-Dokay.staging=off` / `OKAY_STAGING=off` | the codec seam at boot: `Serve` installs okay-staging's generated codecs for every generic door (sessions, Live state, configs, JSON bodies) and prints which way it went; the switch keeps the interpreted fold, nothing else changes (specs/codecs.md, "The codec seam") |
