...
class TestRowAliasFromScala2 extends munit.FunSuite {
  ...
  test("A ! R is Eff[R, A], both ways; every infix type operator has one precedence, so A ! (R + S) needs its parentheses") {
    ...
    assertEquals(!.run(State.handle(4)(Reader.run("env")(wider))), (4, 4))
  }
  ...
  test("F % A is F[A]; in a row each % needs its own parentheses, because a chain is a kind error") {
    ...
    assertEquals(!.run(State.handle(3)(prog)), (3, 3))
  ...
  test("a program at a + row of a user effect and two built-ins, handled effect by effect") {
    ...
    val handled = Console.handle(prog)(a => pure(a))(console)
    assertEquals(!.run(Writer.run(State.handle(0)(handled))), (Vector("got ada"), (3, "ada")))
    assertEquals(!.run(State.handle(0)(Writer.run(handled))), (3, (Vector("got ada"), "ada")))
  ...
  test("a program needing one capability is a program in any + row that has it") {
    ...
    assertEquals(!.run(Throws.runEither(State.handle(4)(Reader.run("env")(wider)))), Right((4, 4)))
  }
  ...
  def viaAliases[R, A](p: A ! (State[Int] + R)): (Int, A) ! R = State.handle[Int, A, R](0)(p)
  ...
  test("a helper polymorphic in the rest of the row: Eff[F with R, A] infers R anywhere; the aliases in a PARAMETER do not") {
    assertEquals(!.run(Writer.run(Reader.run(1)(countFrom(stateFirst)))), (Vector.empty[String], (0, 0)))
    assertEquals(!.run(Writer.run(Reader.run(1)(countFrom(stateMiddle)))), (Vector.empty[String], (0, 0)))
    assertEquals(!.run(Writer.run(Reader.run(1)(countFrom(stateLast)))), (Vector.empty[String], (0, 0)))
    ...
    assert(compileErrors("!.run(Writer.run(Reader.run(1)(viaAliases(stateFirst))))").contains("type mismatch"))
    assert(compileErrors("!.run(Writer.run(Reader.run(1)(viaAliases(stateLast))))").contains("type mismatch"))
  }
}
