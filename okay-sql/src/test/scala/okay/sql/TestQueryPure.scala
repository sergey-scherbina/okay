...
/**
 * specs/optics-outside.md stage 9 — the query's two interpreters read
 * one declaration: the rendered SQL and the in-memory test agree by
 * construction, and a field is refused by name and by type where it
 * is declared.
 */
class TestQueryPure extends munit.FunSuite {
  ...
  test("a field is refused by NAME and by TYPE at construction, naming the fault") {
    ...
    assertEquals(Query.field[Customer, Int]("age").isRight, true)   // an Option[Int] field takes an Int
  }
  ...
  test("RUN: the same predicate in memory — three-valued NULL, LIKE, numbers, negation") {
    assertEquals(all.filter((name like "a%").test), Vector(ann))
    assertEquals(all.filter((age >= 18).test), Vector(ann, cy))           // bob's NULL age compares false
    assertEquals(all.filter((age < 18).test), Vector.empty)               // ... in both directions
    assertEquals(all.filter(age.isNull.test), Vector(bob))
    assertEquals(all.filter((!(age >= 30)).test), Vector(ann, bob))       // NOT of unknown is unknown → false? NO: SQL's NOT NULL is NULL, kept false here
    assertEquals(all.filter(((balance < 0.0) or active.===(true)).test), Vector(ann, bob, cy))
    assertEquals(all.filter((name like "c_ril").test), Vector(cy))
    assertEquals(all.filter((name like "%b%").test), Vector(bob))
    assertEquals(all.filter(Query.Where.all[Customer].test), all)
  }
  ...
  test("UPDATE: the statement, its parameters (sets, then where), and the same edit in memory") {
    ...
    // the two roads on a set of rows: those the WHERE selects are edited, the rest kept
    val w = id === 2L
    assertEquals(all.map(c => if w.test(c) then u(c).toOption.get else c), Vector(ann, bob.copy(balance = 1.0, active = false), cy))
  }
}
