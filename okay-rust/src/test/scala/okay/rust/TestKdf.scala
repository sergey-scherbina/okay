...
object TestKdf:
  ...
  def bouncy(op: Kdf.Argon2id): Either[String, Array[Byte]] =
...
class TestKdf extends munit.FunSuite {
  import TestKdf.*

  test("a program asks Kdf for Argon2id; the handler decides what computes it") {
    given Handler[Kdf] = Kdf.using(op => Right(op.salt.reverse))
    assertEquals(stored("pw", "ab").runWith, Right("6261"))
  }

  test("under the JVM implementation, the program answers Argon2id itself") {
    given Handler[Kdf] = Kdf.using(bouncy)
...
class TestKdfRust extends munit.FunSuite {
  import TestKdf.*
  ...
  private def rust: Handler[Kdf] = Kdf.rust(lib).fold(why => fail(why), identity)
  ...
  test("THE LAW: the Rust kernel's bytes are BouncyCastle's bytes") {
    ...
    val differ = cases.filter { op =>
      val native = Kdf.argon2id(op.password, op.salt, op.memoryKb, op.iterations, op.parallelism, op.length)
