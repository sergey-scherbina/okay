 ...
 *
 * A NEAR-COPY of the JVM suite on purpose: the cross suite
 * deliberately never touches `CanBlock` (it is the one thing the two
 * platforms implement with entirely different machinery — Loom parks
 * against wait/notify), so the rule is asserted once per platform
 * against that platform\'s own implementation.
 */
