...
### M2 — iOS (ui-mobile-ios, LANDED 2026-09-09)

`okay-swift/`, a SwiftPM package with NO okay dependency: `OkayProtocol`
(the document transcribed as enums with associated values, a total
codec over Foundation's JSONSerialization, patch application, the
hybrid rule), `OkayUI` (level L in SwiftUI, `OkayApp(url:)` as the
whole client in one view, `Client` over `URLSessionWebSocketTask` —
hello first, events queued until the socket opens, the lesson of M1),
and `okay-smoke`, the headless check.

- [x] `swift test` replays `docs/protocol/conformance.jsonl`: every
      `in` applied, every `tree` held, every `out` reproduced, the
      Submitted produced by the hybrid rule from the same edits — 3 of 3
- [x] `xcodebuild -scheme OkayUI -destination 'generic/platform=iOS
      Simulator' build`: BUILD SUCCEEDED, arm64 and x86_64 simulator
      slices of OkayProtocol and OkayUI
- [x] the same server drives it: `okay.script.Serve` served the Live
      counter page, `swift run okay-smoke` opened its WebSocket,
      received the tree, pressed `inc`, held `count: 1` after the patch
- [ ] an app bundle: an iOS app is an Xcode project that adds this
      package and shows `OkayApp(url:)` — the project file is the
      operator's to create in Xcode; not generated by hand here

### M3 — Android (ui-mobile-android, LANDED 2026-09-09)

The SDK by `brew install --cask android-commandlinetools` and
`sdkmanager` (platform-tools, platforms;android-35, build-tools
35.0.0 into `~/Library/Android/sdk`). `okay-compose/app` is a Kotlin
Multiplatform module now: ONE set of composables in `commonMain`
(`Render.kt`, `App.kt` with `OkayApp(url)`, and `Client.kt` — the
session, hello first, events queued until the socket opens), and only
the SOCKET per platform (`expect class Socket`: the JDK's WebSocket on
the desktop, OkHttp on Android, which has no `java.net.http`).
`MainActivity` shows `OkayApp` with the server address from the launch
intent (`--es url ...`), `10.0.2.2` — the emulator's host — by default.

- [x] `./gradlew :app:assembleDebug` builds `app-debug.apk` (8.5 MB)
      against compileSdk 35 / minSdk 26; the desktop target compiles
      from the same sources; `:protocol:test` 3 of 3 unchanged
- [x] the desktop smoke against a real `okay.script.Serve` page still
      passes with the rewritten client (the socket behind expect/actual)
- [ ] running the APK: an emulator system image is a further download
      (`sdkmanager "system-images;android-35;google_apis;arm64-v8a"`,
      `avdmanager create avd`, `emulator`); not run in this session —
      the README gives the `adb install` / `am start` lines
- Found: a `private fun raw(m) = socket.send(...)` whose type Kotlin
  inferred from a lambda that also referenced it — "type checking has
  run into a recursive problem"; explicit `Unit` return types. And by
  the smoke, not the build: the JDK WebSocket's `onOpen` fires INSIDE
  `buildAsync().join()`, before the field the socket is assigned to
  exists, so the hello never left and no tree ever came — the socket
  is taken from the callback on both platforms now
