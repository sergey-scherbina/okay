...
/** the typed-row builder (Row.scala): a compile-time-checked partial
 * UPDATE/INSERT with no case class declared. */
class TestRow extends munit.FunSuite {
  ...
  test("columns encode in insertion order") {
    ...
    assertEquals(row.toParams, Right(Vector(
  ...
  test("a wide row keeps every column, none dropped or reordered") {
    ...
    assertEquals(row.toParams, Right(Vector(
      "name" -> SqlValue.Text("ada"),
      "age" -> SqlValue.I32(28),
      "balance" -> SqlValue.F64(12.5))))
  }

  test("the row's TYPE is exactly its columns: reading one never set does not compile") {
    assert(compileErrors(
      """okay.sql.Row.empty.updated(okay.sql.Column[String]("name"), "grace").get(okay.sql.Column[Int]("age"))"""
    ).nonEmpty)
  }
}
