...
final case class PyEnv(python: String = "3.12",
  ...
  def start(env: Map[String, String] = Map.empty, modules: Seq[PyModule] = Nil)(using WireFormat, WireCompression, WireDeadline): ForeignWorker =
    ...
    if drift.isEmpty then w
      ...
      w.close()
      throw IllegalStateException(s"okay.foreign: the provisioned environment is not the declared one:\n  ${drift.mkString("\n  ")}")

object PyEnv:
  private[foreign] def run(cmd: Vector[String], what: String): Unit =
    val pb = ProcessBuilder(cmd*).redirectErrorStream(true)
    val p =
      try pb.start()
      catch case e: java.io.IOException =>
        throw IllegalStateException(s"okay.foreign: could not $what: '${cmd.head}' did not start (${e.getMessage}) — is uv installed?")
    val out = String(p.getInputStream.readAllBytes(), java.nio.charset.StandardCharsets.UTF_8)
    val code = p.waitFor()
    if code != 0 then
      throw IllegalStateException(s"okay.foreign: could not $what (exit $code):\n${out.linesIterator.toVector.takeRight(12).mkString("\n")}")

  private[foreign] def delete(d: Path): Unit =
