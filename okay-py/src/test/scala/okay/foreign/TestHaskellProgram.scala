...
class TestHaskellProgram extends munit.FunSuite {
  ...
  test("a Haskell program's operation is a Scala callback under the caller's Reader") {
    val price = Foreign.callback[String, Double]("price_of")(sku => Reader.ask[Map[String, Double]].map(_(sku)))
    val run = Foreign.program[Double]("priced").calling(Foreign.callbacks(price))("tea", 3L)
  ...
  test("a Haskell error is a condition by name, and the worker lives on") {
    ...
    val again = Foreign.program[Long]("pairs").calling(Foreign.callbacks(choose))()
