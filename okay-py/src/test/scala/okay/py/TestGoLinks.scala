...
object GoWorkerBinary:
  ...
  lazy val binary: Path =
    ...
    java.nio.file.Files.writeString(dir.resolve("shop").resolve("ops.go"),
      Go.ops("shop", Py.callbacks(TestGoProgram.priceOf, TestGoProgram.discount))): Unit
  ...
  def serveTcp(): (PySubprocess, Process) =
    ...
    val port = okay.codec.Json.parse(first) match
      ...
      case _ => None
    port match
      case Some(n) => (PySubprocess.connect("127.0.0.1", n), p)
...
class TestGoPipes extends WireConformance:
  ...
  lazy val engine: PySubprocess = PySubprocess.speaking(Seq(GoWorkerBinary.binary.toString))
...
class TestGoTcp extends WireConformance:
  ...
  lazy val engine: PySubprocess = served._1
