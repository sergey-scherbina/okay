- [ ] foreign-held-values — `Objects[L]` for the Go, Rust, Haskell and
      TypeScript libraries: a small table of held values, so `call … held`
      answers a ref and `release` drops it (specs/foreign-one.md stage 6 as
      first written; narrowed by Decision 18). Python and R hold today;
