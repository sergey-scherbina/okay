...
class TestWriter extends munit.FunSuite {
  ...
  test("run collects everything told, in order, forwarding the rest") {
    ...
    assertEquals(Writer.run(p).runWith, (Seq("a", "b"), 1))
    assertEquals(Writer.collect(p).runWith, (Vector("a", "b"), 1))
  }
  ...
  test("foldWith folds the told values") {
    ...
    assertEquals(!.run(Writer.foldWith(p)(0)(_ + _)), (5050, ()))
  }
  ...
  test("stack safety: 1M tells") {
    ...
    assertEquals(!.run(Writer.foldWith(p)(0L)(_ + _))._1, n.toLong * (n + 1) / 2)
  }
  ...
  test("map transforms the telling in place and forwards the rest") {
    ...
    val q: Int ! (Writer[String] + Produce) = Writer.map(p)(i => "n" + i)
    assertEquals(Writer.run(q).runWith, (Seq("n1", "n5"), 5))
  }
}
...
class TestThrows extends munit.FunSuite {
  ...
  test("raise and runEither") {
    ...
    assertEquals(!.run(Throws.runEither(ok)), Right(1))
    assertEquals(!.run(Throws.runEither(bad)), Left("no"))
  }
  ...
  test("runUnsafe: the JVM is the handler") {
    ...
    intercept[RuntimeException](!.run(Throws.runUnsafe(p)))
  }
...
class TestReader extends munit.FunSuite {
  ...
  test("ask answers the environment; local modifies it for a sub-program") {
    ...
    val p: Int ! Row = for {
      ...
      b <- Reader.local((_: Int) * 10)(Reader.ask[Int].at[Row])
      c <- produce(1).at[Row]
    } yield a + b + c
