 ...
 */
/**
 * `Journalled` moved to okay-codec (`okay.codec.Journalled`,
 * foreign-journalled, specs/foreign-highlevel.md stage 1), so that an
 * operation declared in a module that does NOT depend on okay-agent —
 * okay-py's `PyEval`, okay-r's `REval` — can carry its own instance.
 * The name stays here for every caller that already wrote it, and the
 * `Tool` instance lives in `Tool`'s companion, where implicit scope
 * finds it without an import.
 */
