...
import okay.!.*
import okay.RowLift.{plus, bindIn, thenIn}
 ...
 */
class TestBindIn extends munit.FunSuite:
  ...
  test("the other row is read off the continuation; the same answer and log as the two-plus spelling") {
    val viaBindIn: Int ! Two = Reader.ask[Int].bindIn(e => Writer.tell(s"env=$e").map(_ => e + 1))
  ...
  test("three rows, two binds: the union of all three, effects in order") {
    val p: Int ! (Reader % Int + Writer % String + State % Int) =
      Reader.ask[Int]
        .bindIn(e => Writer.tell(s"saw $e"))
        .bindIn(_ => State.modify[Int](_ + 1))
  ...
  test("the tree is one Bind whose continuation is f — no walk, no re-injection") {
    var asked = 0
    val p: Int ! Two = Reader.ask[Int].bindIn(e => Writer.tell(s"$e").map(_ => e))
