...
object TsFacade:
  ...
  def render(obj: String, pkg: String, module: String, dts: String, imports: Seq[String] = Nil): Either[String, String] =
    ...
    yield
      ...
      val py = (if made.nonEmpty then Vector("Condition", "PyEval", "Ts") else Vector.empty) ++ Option.when(needsToPy)("ToPy")
      ...
      for f <- methods do
        ...
        f match
          ...
          case Right((fn, Right(mm))) =>
            ...
            b ++= s"  def ${ident(fn.name)}$tpClause(${mm.params.mkString(", ")}): Either[Condition, ${mm.out}] ! PyEval =\n"
