...
object PyModule:
  ...
  def fromConstant(name: String, source: String): PyModule =
    require(Identifier.matches(name), s"okay.py: a module name is a Python identifier, got '$name'")
   ...
   */
  private[py] def env(modules: Seq[PyModule], env: Map[String, String]): Map[String, String] =
    if modules.isEmpty then env
    else
      val names = modules.map(_.name)
      require(names.distinct.size == names.size, s"okay.py: two modules share a name: ${names.mkString(", ")}")
