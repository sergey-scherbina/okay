...
/**
 * MCP push over the Netty backend (http-streaming-responses): the
 * same TestMcpPush okay-jetty already ran, proving the gap `Netty`'s
 * own REST handler had (it aggregated the whole response, so a
 * subscription push waited behind a body that never ended) is closed.
 */
class TestMcpPush extends munit.FunSuite {
  ...
  val info = Mcp.Info("okay-push-netty", "0.1")
  ...
  /** a Netty server on a free port, for the body of the test */
  def served[A](route: Request => Response ! Async)(body: String => A): A =
    Resource.run[A, Pure](Netty.serve(0)({ case r if true => route(r) })()
      .map(s => body(s"http://127.0.0.1:${Netty.port(s)}/mcp"))).runWith
