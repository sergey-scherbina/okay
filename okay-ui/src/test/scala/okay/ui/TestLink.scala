...
class TestLink extends munit.FunSuite:
  ...
  // ---- react-host-vocab: a host lowers with its OWN vocabulary ------

  /** every level-S node the browser does NOT claim, each holding the
   * one it does. The catch-all used to ask for these with `Set.empty`,
   * and `Ui.lower` recurses, so the link inside was lowered away. */
  private val nested: Vector[(String, Ui)] = Vector(
    "Table" -> Ui.Table(Vector("where"), Vector(Vector(link)), "t"),
    "Items" -> Ui.Items(Vector(link), "i"),
    "Tabs" -> Ui.Tabs(Vector("one"), 0, Vector(link), "tb"),
    "Modal" -> Ui.Modal("title", link, "m"),
    "Disclosure" -> Ui.Disclosure("more", true, link, "d"))

  test("a Link the browser claims survives a node it does not: an anchor in every level-S container") {
    // the tag, not the whole tag TEXT: a Table's lowering weights its
    // cells, so the anchor there also carries `style="flex:1"`
    for (name, tree) <- nested do
      assertEquals(clue(Html.render(tree)).contains("""<a href="/api/cases.csv?of=cases""""), true,
        s"$name lowered the link away")
  }

  test("THE TWO ROADS AGREE ON ONE TREE, which is why this matters") {
    // `live.js` says `vocab: ["link"]` in its hello, so `Wire` sends
    // the socket a tree lowered with THAT — while `Html.render` is the
  ...
  test("a host that claims NOTHING is untouched: the terminal still shows the label and the URL") {
    for (_, tree) <- nested do
      val frame = Frame.render(Ui.Column(Vector(tree)), None).mkString("\n")
      assert(frame.contains("cases .csv") && frame.contains("/api/cases.csv"), frame)
  }
