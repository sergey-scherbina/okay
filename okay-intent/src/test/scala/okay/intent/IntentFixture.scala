...
object IntentFixture {
  ...
  val parallel: List[Parallel] = List(
    ...
    Parallel("book-room", "Request", Map(
      ...
      "uk" -> "Могли б ви забронювати кімнату на чотирьох?",
      "pl" -> "Czy mógłbyś zarezerwować salę dla czterech osób?")),
    Parallel("confirm-attend", "Request", Map(
      ...
      "pl" -> "Proszę potwierdzić, czy będziesz obecny.")),
    Parallel("room-changed", "Notification", Map(
      ...
      "ja" -> "会議室がB2に変更されました。",
      "uk" -> "Кімнату для зустрічі змінено на B2.",
      "pl" -> "Sala spotkań została zmieniona na B2.")),
    ...
    Parallel("propose-weekly", "Proposal", Map(
      ...
      "ja" -> "毎週月曜日に定例を追加しませんか。",
      "uk" -> "Може, додамо щотижневу зустріч по понеділках?",
    ...
    Parallel("request-slides", "Request", Map(
      ...
      "uk" -> "Надішліть, будь ласка, слайди до четверга.",
      "pl" -> "Proszę przesłać slajdy przed czwartkiem.")),
    Parallel("request-invite", "Request", Map(
      ...
      "ja" -> "招待に私を追加していただけますか。",
      "uk" -> "Могли б ви додати мене до запрошення?",
      "pl" -> "Czy możesz dodać mnie do zaproszenia?")),
    ...
    Parallel("request-room", "Request", Map(
      ...
      "uk" -> "Можете перевірити, чи вільна кімната?",
      "pl" -> "Czy możesz sprawdzić, czy sala jest wolna?")),
    ...
    Parallel("notify-remote", "Notification", Map(
      ...
      "ja" -> "今後、木曜日はリモートになります。",
      "uk" -> "Відтепер четверги — віддалені.",
      "pl" -> "Od teraz czwartki są zdalne.")),
    ...
    Parallel("other-recipe", "Other", Map(
      ...
      "pl" -> "Oto przepis, o który pytałeś przy obiedzie.")),
    ...
    Parallel("other-password", "Other", Map(
      ...
      "uk" -> "Моє посилання для скидання пароля протерміноване.",
