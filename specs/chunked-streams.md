...
`Feed[Chunk[A]]` (a pure writer stream; `Producer[Chunk[A]]` until
producer-to-writer-carrier, 2026-09-19), so every existing consumer
(iterator, merge, Channel, fold) works on it unchanged, because elements
are polymorphic.

## Interface
- `type Chunk[+A] = scala.collection.immutable.ArraySeq[A]`
- `type Chunks[A] = Feed[Chunk[A]]` — a stream of batches, told one chunk at a
  time (was `Producer[Chunk[A]]`; retyped by producer-to-writer-carrier)
...
- **`Chunks[A]` is an alias of an existing carrier, not a new one** (first
  `Producer[Chunk[A]]`, since 2026-09-19 `Feed[Chunk[A]]`, see
  specs/producer-to-writer-carrier.md) — the whole
