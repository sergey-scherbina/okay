 ...
 *
 * A `Staged[Row, R, A]` is `Func` — the program as a function of its
 ...
 * F. Its operations are not dispatched: a `Stage` object's `stage` is
...
object Staged:
  ...
  inline def shift[Row[+_], R, A](inline f: (A => R) => R): Staged[Row, R, A] = f

  inline def pure[Row[+_], R, A](a: A): Staged[Row, R, A] = k => k(a)

  /** apply the program to its final continuation */
  inline def run[Row[+_], R, A](p: Staged[Row, R, A])(k: A => R): R = p(k)
  ...
  given [Row[+_], R]: Monad[Staged[Row, R, *]] with
    override inline def pure[A](a: A): Staged[Row, R, A] = k => k(a)
    override inline def fmap[A, B](m: Staged[Row, R, A], f: A => B): Staged[Row, R, B] =
      k => m(x => k(f(x)))
    extension [A](m: Staged[Row, R, A])
      override inline def flatMap[B](f: A => Staged[Row, R, B]): Staged[Row, R, B] =
        k => m(f(_)(k))

type Staged[Row[+_], R, A] = Staged.Staged[Row, R, A]

/**
 * A row's staged interpreter: ONE object (or class instance) per row
 * and answer layout, whose `stage` is
 *
 *   inline def stage[X](inline op: Row[X]): Staged[Row, R, X] =
 *     inline op match
 ...
 * than summoning a `Stage[Row, R]` that would widen to this trait.
 */
trait Stage[Row[+_], R]
...
object Stage:
  ...
  final class StateWriter[S, W, A] extends Stage[State % S + Writer % W, Answer[S, W, A]]:
    type Row = State % S + Writer % W
    type R = Answer[S, W, A]

    inline def stage[X](inline op: Row[X]): Staged[Row, R, X] =
      inline op match
        case State.Get() => Staged.shift[Row, R, X](k => st => k(st._1)(st))
        case State.Set(s2) => Staged.shift[Row, R, X](k => st => k(s2)((s2, st._2)))
        case Writer.Say(v) => Staged.shift[Row, R, X](k => st => k(())((st._1, st._2 :+ v)))

    /** run a block from an initial state: the state and the log in
     * the row's layout, then the answer */
    def run(s: S)(p: Staged[Row, R, A]): ((S, Vector[W]), A) =
      Staged.run(p)(a => acc => (acc, a))((s, Vector.empty))
