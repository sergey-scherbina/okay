...
import okay2.stream.Pipe.{into, through}

/** okay-parse's TestParse on okay2: total parsing — any input yields a
 * tree, errors are data in it; incremental reparse reuses subtrees */
class TestParse extends munit.FunSuite {
  ...
  def parse(s: String, d: Parse.Driver[K, Unit] = JsonParse.driver): Cst[K] =
    Parse.toCst(new FeedOps(into(chars(s))(through(Scan.stage(JsonLex.scan))(d))).toLazyList)
  ...
  test("sibling recovery: one bad token damages one leaf, not the tree") {
    ...
    assertEquals(Cst.errors(t).map(_._2), Vector("unexpected '@'"))
  ...
  test("a close with nothing open is an error leaf, not a fault") {
    ...
    assertEquals(Cst.errors(t).map(_._2), Vector("nothing to close", "nothing to close"))
  }

  test("the two surfaces produce the SAME tree (the convergence contract)") {
    for (s <- List(sample, "{\"a\": [1, 2", "[1, @, 3]", "}]", ""))
  ...
  def gammaNode(t: Cst[K]): Cst[K] = t match {
    case Cst.Node(_, rootKids) => rootKids.headOption match {
      case Some(Cst.Node("object", kids)) => kids.collectFirst { case n @ Cst.Node("object", _) => n }.get
      case other => fail(s"no root object: $other")
    }
    case other => fail(s"no root: $other")
  }
  ...
  test("incremental reparse: a length-preserving edit reuses subtrees by reference") {
    ...
    assert(steps < old.lexed.tokens.length / 2, s"not incremental: $steps of ${old.lexed.tokens.length} tokens re-driven")
    assert(gammaNode(re.tree) eq gammaNode(old.tree), "the untouched subtree must be the SAME object, not a rebuild")
  }

  test("incremental reparse: a length-changing edit still equals the full parse") {
    val old = Parse.full(JsonLex.scan, JsonParse.instrs)(doc, snapshotEvery = 8)
    val edited = doc.replace("123", "1")
    val at = doc.indexOf("123")
    val re = Parse.reparse(JsonLex.scan, JsonParse.instrs)(old, doc, edited, at, at + 3, at + 1, 8)
    assertEquals(re.tree, Parse.full(JsonLex.scan, JsonParse.instrs)(edited).tree)
    assertEquals(Cst.lexemes(re.tree), edited)
  }

  test("depth: a 20 000-deep document parses losslessly (the stack is a List, sizeIs, not length)") {
    val n = 20000
    val s = ("{\"kids\":[" * n) + "{\"kids\":[]}" + ("]}" * n)
